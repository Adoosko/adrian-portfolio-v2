...
export const ScrollTextReveal: FC<ScrollTextRevealProps> = ({ 
  ...
  const { scrollYProgress } = useScroll({
    target: targetRef,
    // ✅ Ešte dlhší offset pre fade-out efekt
  ...
  const { startDelay, endDelay, wordSpacing, fadeOutStart } = speedSettings[revealSpeed];
      ...
      <div className="mx-auto max-w-4xl px-6">
        <div className="text-lg leading-relaxed md:text-xl lg:text-2xl">
          {words.map((word, i) => {
            ...
            const fadeOutOpacity = enableFadeOut 
              ? useTransform(
                  scrollYProgress,
            ...
            // Blur pre fade-out
            const fadeOutBlur = enableFadeOut
              ? useTransform(
                  scrollYProgress,
                  [fadeOutStartPoint, fadeOutEnd],
                  [0, blurStrength]
